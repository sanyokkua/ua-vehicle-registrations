...
public class FileUtilsService {
    ...
    public Optional<Path> getTempDirectory() {
        ...
        if (createdFolder.isEmpty()) {
            log.warn(
                    "getTempDirectory: Was the problem with creating temp directory, trying to create in the app " +
                            "directory");
    ...
    public String getEncoding(@NonNull Path textFile) {
        try {
            ...
            log.warn("getEncoding: Exception occurred due encoding detecting. Default value \"UTF-8\" will be returned",
                    e
            );
            return "UTF-8";
        }
